...
MODEL_ID_AMAZON_NOVA = "MODEL ID"  # amazon nova pro
MODEL_ID_CLAUDE_SONNET = "MODEL ID"  # claude 3.5 sonnet v2
SYSTEM_PROMPT = """
...
def structured_output_claude():
    return {"toolSpec": {"name": "for_structured_output",
                         "description": "入力された音声通話テキストに対して、「Yes」か「No」を返すツール",
                         "inputSchema": {"json": {"type": "object",
                                                  "properties": {"speech_to_text": {"type": "string",
                                                                                    "description": "音声通話テキスト"},
                                                                 "result": {"type": "string",
                                                                            "enum": ["Yes", "No"],
                                                                            "description": "音声通話テキストの判断結果"}},
                                                  "required": ["speech_to_text"]}}}}


def call_claude_sonnet(input_text_data):
    USER_PROMPT = input_text_data
    prompt = """
    {a}
    <text>
    ...
    """.format(a=SYSTEM_PROMPT, b=USER_PROMPT)
    claude_sonnet_input_with_prompt = [{"role": "user",
                                        "content": [{"text": prompt}]}]
    claude_sonnet_response = BEDROCK_CLIENT.converse(modelId=MODEL_ID_CLAUDE_SONNET,
                                                     messages=claude_sonnet_input_with_prompt,
                                                     inferenceConfig={"temperature": 0.2},
                                                     toolConfig={"tools": [structured_output_claude()],
                                                                 "toolChoice": {"tool": {"name": "for_structured_output"}}})
    claude_sonnet_response_result = claude_sonnet_response["output"]["message"]["content"][0]["toolUse"]["input"]["result"]
    return claude_sonnet_response_result